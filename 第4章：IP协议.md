...
IPv6 的 IP 地址长度为 128 位。一般人们将 128 比特 IP 地址以每 16 比特为一组，每组用冒号 ":" 隔开进行标记。而且如果出现连续的 0 时还可以将这些 0 省略，并用两个冒号 "::" 隔开。但是，一个 IP 地址中只允许出现一次两个连续的冒号。在 IPv6 当中，人们正在努力使用最简单的方法标记 IP 地址，以便易于记忆。
